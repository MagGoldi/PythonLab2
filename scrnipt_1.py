...
def run_1(path_to_csv: str=os.path.join("C:/", "PYTHON", "PythonLab2", "File_folder")) -> None:
    ...
    with open(path_to_csv + "/scrnipt_1/X.csv", 'w', newline='', encoding='cp1251') as csvfile_x:
        for i in range(0, len(list1)):
            all_data = [list1[i][0]]
            writer = csv.writer(csvfile_x)
            writer.writerow(all_data)

    with open(path_to_csv + "/scrnipt_1/Y.csv", 'w', newline='', encoding='cp1251') as csvfile_y:
        for i in range(0, len(list1)):
            all_data = list1[i][1:]
