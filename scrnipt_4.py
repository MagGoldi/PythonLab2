...
def work_1(date,path_to_csv):
    with open(path_to_csv+'/scrnipt_1/X.csv', 'r', newline='', encoding="utf-8") as csvfile:
        file_reader = list(csv.reader(csvfile))
        for i in range(len(file_reader)):
            ...
            return None
    with open(path_to_csv+'/scrnipt_1/Y.csv', 'r', newline='', encoding="utf-8") as csvfile:
        file_reader = list(csv.reader(csvfile))
...
def work_2(date, path_to_csv):
    ways = os.listdir(path=path_to_csv + "/scrnipt_2")
    date = str(date)
...
def work_3(date, path_to_csv):
    ways = os.listdir(path= path_to_csv+"/scrnipt_3")
    list1 = []
    date = str(date)
...
def run_4(path_to_csv=os.path.join("C:/","PYTHON", "PythonLab2-1", "File_folder")):
