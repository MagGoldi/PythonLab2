...
def work_1(date):
    with open('File_folder/scrnipt_1/X.csv', 'r', newline='') as csvfile:
    ...
    with open('File_folder/scrnipt_1/Y.csv', 'r', newline='') as csvfile:
        file_reader = list(csv.reader(csvfile))
...
def work_2(date):
        ...
        for i in range(len(ways)):
            if(ways[i][:4]==date[:4]):            #Ищем файл у которого аткой же год
                with open("File_folder/scrnipt_2/" + ways[i], 'r', newline='') as csvfile:
                    file_reader = csv.reader(csvfile)
                    for row in file_reader:
                        if(row[0] == date):
                            print(*row)
...
def work_3(date):
    ...
    for i in range(len(list1)):
        if(int(list1[i][11:13]) >= int(date[8:10]) >= int(list1[i][8:10])): #если число находится в диапозоне недели то выводим его
            with open("File_folder/scrnipt_3/" + list1[i], 'r', newline='') as csvfile:
                file_reader = csv.reader(csvfile)
                date = re.sub(r'[_]', '-', date)
                for row in file_reader:
                    if(row[0] == date):
